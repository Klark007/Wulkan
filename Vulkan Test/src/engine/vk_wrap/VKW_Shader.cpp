...
void VKW_Shader::init(const VKW_Device* vkw_device, const std::string& path, VkShaderStageFlagBits stage, const std::string& entry_func)
{
	device = vkw_device;
	...
	if (!file.is_open()) {
		throw IOException("Failed to open shader file at " + path, __FILE__, __LINE__);
	}
	...
	VK_CHECK_ET(vkCreateShaderModule(*device, &create_info, VK_NULL_HANDLE, &module), RuntimeException, "Failed to create shader module from path " + path);
